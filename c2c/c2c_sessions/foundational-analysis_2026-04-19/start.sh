#!/bin/bash
# C2C launcher — foundational analysis session (Session 1 of 3).
# Clean shutdown via signal traps so stop.sh doesn't leave orphan sleeps.

set -e

SESSION_DIR="/Users/june/Documents/GitHub/liberation_labs/c2c_sessions/foundational-analysis_2026-04-19"
...
# Launch Instance A (no offset) — Opus 4.7 leads in analytical cycles
bash -c 'run_loop a "$MODEL_A" "$PROMPT_A" 0' &
echo $! >> "$PID_FILE"

# Launch Instance B (offset) — Sonnet 4.6 as counterpart-reader / stress-tester
bash -c "run_loop b \"\$MODEL_B\" \"\$PROMPT_B\" $OFFSET_SECONDS" &
echo $! >> "$PID_FILE"

echo "C2C foundational analysis session started."
echo "  Instance A (Opus, analytical lead):       PID $(sed -n 1p "$PID_FILE") — first cycle firing NOW"
echo "  Instance B (Sonnet, counterpart-reader):  PID $(sed -n 2p "$PID_FILE") — first cycle in ~15 min"
echo ""
echo "Watch logs:"
